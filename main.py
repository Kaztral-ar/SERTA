#!/usr/bin/env python3
"""
SERTA v1 — Universal Server Builder
Folder upload · Bot Hosting · Web · Storage
"""
import sys,os,json,time,socket,shutil,secrets,hashlib,hmac,logging
...
def banner():
    ...
    print(f"{C.BYELLOW}{C.BOLD}{'Server Creation & Automation · Universal Server Builder v1'.center(TERM_WIDTH)}{C.RESET}")
...
def check_password(pw, stored):
    ...
    legacy = hashlib.sha256(f"serta_v1_salt{pw}".encode()).hexdigest()
...
def create_project():
    ...
    stype=menu("Server Type",[
        ...
        ("2","File Storage Server — folder upload, download, manage"),
        ("3","Bot Hosting         — Discord / Telegram / Slack / Generic Python bot"),
    ])
    type_map={"1":"web","2":"storage","3":"bot"}
    server_type=type_map.get(stype,"web")
    port_str=prompt("Port","5000")
    try:port=int(port_str)
    except (ValueError, TypeError): port=5000
...
fetch('/csrf').then(r=>r.json()).then(d=>{
...
def _shell(title,body,nav_active,project_name,extra_nav="",hosted=None,pub_url=None):
    ...
    return (f'<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8">'
            ...
            f'<div class="sidebar-footer">Serta v1</div>'
...
def _login_page(error="",action="/admin/login"):
    return (f'<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8">'
            ...
            f'<input type="hidden" name="_csrf_token" value="">'
            ...
            f'<script>fetch("/csrf").then(r=>r.json()).then(d=>{{document.querySelector("input[name=_csrf_token]").value=d.token}}).catch(()=>{{}})</script>'
...
def main():
    ...
    while True:
        ...
        if choice=="1":
            ...
            print(f"\n  {C.BGREEN}Goodbye!{C.RESET}  Serta v1\n");sys.exit(0)
        else:warn("Invalid choice.")
        input(f"\n  {C.DIM}Press Enter…{C.RESET}")
